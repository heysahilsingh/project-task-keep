...
function editUserName(cureentUserName) {
  const input = createElement("input", {
    class: "edit-user-name-input",
    type: "text",
    placeholder: "Type Your Name",
    value: cureentUserName,
  });
...
function addList(heading, name, id, btn_1, btn_2, callback) {
  ...
  dialogBox.querySelector(".box-btns .btn-2").addEventListener("click", () => {
    ...
    } else if (dbLists.some((list) => list.listName === enteredValue)) {
      message("List already exists.");
    } else {
      dbAddList(enteredValue, id);
      lightBox.classList.remove("active");
      dialogBox.remove();
...
function addTask(
  ...
  dialogBox.querySelector(".box-btns .btn-2").addEventListener("click", () => {
    ...
    else if (dbTasks.some((task) => task.taskName === nameOption.value)) {
      message("Task already exist");
    }
    else if (!nameOption.value.trim()) {
      message("Please enter a task name");
    } 
    else if (!dateOption.value.trim()) {
      message("Please select a task complete date");
    } 
    else if (!document.querySelector(".light-box .list-wrapper")) {
